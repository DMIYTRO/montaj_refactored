...
### Добавлено
- Автоматическая обработка больших тиражей (>500 листов)
- Папка `big_print/` для файлов с большим тиражом
- Счетчик `bigPrintCount` в статистике обработки
- Уведомление о больших тиражах в финальном алерте
- Логирование файлов с большим тиражом

### Изменено
- **КРИТИЧНО:** Удален диалог подтверждения для тиражей >500
  - Ранее: показывался диалог с возможностью редактирования тиража
  - Теперь: файл автоматически перемещается в `big_print/` без раскладки
- Улучшена логика обработки больших заказов
- Обновлен `.gitignore` с добавлением папки `big_print/`

### Поведение
- Если листовой тираж > 500:
  1. Закрывается открытый документ InDesign (без сохранения)
  2. Файл перемещается в `sourceFolder/big_print/`
  3. Раскладка НЕ выполняется
  4. Логируется информация о тираже и файле
  5. Скрипт переходит к следующему файлу
...
### [2.2.0] - Планируется
- [ ] Настраиваемый порог большого тиража
...
[2.1.0]: https://github.com/DMIYTRO/montaj_refactored/releases/tag/v2.1.0
